...
function countAllMdFiles(dir: string): number {
  if (!existsSync(dir)) return 0;
  let count = 0;
  try {
...
export async function run(): Promise<void> {
  ...
  if (tasks.length === 0) {
    ...
    console.log(
      chalk.dim(
        `    ${"Name".padEnd(nameWidth + 2)}${"Mode".padEnd(modeWidth + 2)}${"Phase".padEnd(phaseWidth + 2)}Status`
      )
    );
    ...
    for (const task of tasks) {
      ...
      console.log(
        `    ${chalk.cyan(task.name.padEnd(nameWidth + 2))}${task.mode.padEnd(modeWidth + 2)}${task.phase.padEnd(phaseWidth + 2)}${statusColor(task.status)}`
      );
    }
  }
