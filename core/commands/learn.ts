...
function categorize(filePath: string): CategoryRule | null {
  const lowerPath = filePath.toLowerCase();
  for (const rule of CATEGORY_RULES) {
    for (const pattern of rule.patterns) {
      if (lowerPath.includes(pattern)) return rule;
    }
  }
  return null;
}
...
export async function run(): Promise<void> {
  ...
  console.log();
  console.log(
    chalk.bold.magenta("  Becky learn") +
  ...
  for (const relPath of mdFiles) {
    ...
    console.log(
      `    ${chalk.dim(relPath)} → ${chalk.cyan(relative(BECKY_ROOT, finalPath))}`
    );
  ...
  console.log(
    `    ${chalk.bold(String(mdFiles.length).padStart(4))} total files copied`
  );

  if (memoryCount > 0) {
    console.log(
      `    ${chalk.bold(String(memoryCount).padStart(4))} memory entries created (incidents/postmortems)`
    );
  }

  console.log();
  console.log(
    `  Next: run ${chalk.green("becky compile")} to regenerate CLAUDE.md and AGENTS.md.`
  );
  console.log();
}
