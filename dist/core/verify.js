...
function checkAgents(ruleIds) {
    ...
    for (const filename of files) {
        ...
        // Check required fields
        if (!frontmatter.id)
            addIssue("error", filename, "Agent missing id");
        if (!frontmatter.name)
            addIssue("error", filename, "Agent missing name");
        if (!frontmatter.runtime)
            addIssue("error", filename, "Agent missing runtime");
        if (frontmatter.id)
            agentIds.add(String(frontmatter.id));
