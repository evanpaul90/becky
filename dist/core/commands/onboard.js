...
function hasFolderContents(dir) {
    if (!existsSync(dir))
        return false;
    try {
...
export async function run() {
    ...
    console.log();
    console.log("  Becky is a multi-agent coding OS. Agents collaborate through a shared");
    console.log("  wiki, enforce codified rules, and run structured pipelines to ship");
    ...
    for (const folder of FOLDERS) {
        ...
        console.log(`    ${chalk.cyan(folder.path.padEnd(18))} ${folder.description} [${status}]`);
    }
    // Agent roster
    section("Agent Roster");
    for (const agent of AGENTS) {
        console.log(`    ${chalk.yellow(agent.name.padEnd(12))} ${agent.role}`);
    }
    // Commands
    section("Available Commands");
    const maxLen = Math.max(...COMMAND_LIST.map((c) => c.name.length));
    for (const cmd of COMMAND_LIST) {
        console.log(`    becky ${chalk.green(cmd.name.padEnd(maxLen + 2))} ${cmd.description}`);
    }
    // Config check
    ...
    if (existsSync(configPath)) {
        ...
        if (!projectName || !userName) {
            console.log(chalk.yellow("    becky.config.yaml needs attention:"));
            if (!projectName) {
                console.log(`      - ${chalk.cyan("project.name")} is empty — set your project name`);
            }
            if (!userName) {
                console.log(`      - ${chalk.cyan("user.name")} is empty — set your name so agents can address you`);
            }
            console.log();
            console.log(`    Edit: ${chalk.cyan(configPath)}`);
        }
        else {
            console.log(`    Project: ${chalk.bold(projectName)}  |  User: ${chalk.bold(userName)}`);
        }
    }
    else {
        console.log(chalk.yellow("    becky.config.yaml not found — run ") +
    ...
    const hasTaskFolders = existsSync(tasksDir) &&
        readdirSync(tasksDir).some((e) => !e.startsWith(".") && !e.startsWith("_"));
    if (!hasTaskFolders) {
        console.log("    No tasks found. To get started:");
        console.log(`      ${chalk.green("becky greenfield")}   Start a new project from scratch`);
        console.log(`      ${chalk.green("becky brownfield")}   Onboard an existing codebase`);
    }
    else {
        console.log(`    Tasks found in ${chalk.cyan("tasks/")} — run ${chalk.green("becky status")} to see current state.`);
    }
    // Wiki check
    const compiledDir = join(getWikiDir(), "compiled");
    const hasArticles = existsSync(compiledDir) &&
        ["concepts", "decisions", "incidents"].some((sub) => hasFolderContents(join(compiledDir, sub)));
    if (!hasArticles) {
        console.log();
        console.log("    Wiki is empty. If you have existing docs:");
        console.log(`      ${chalk.green("becky learn ./path/to/docs")}   Import markdown files into wiki`);
    }
    console.log();
}
