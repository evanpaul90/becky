...
function findFileContent(taskDir, pattern) {
...
function collectFeedbackFiles(taskDir) {
    const feedback = [];
    const entries = readdirSync(taskDir, { withFileTypes: true });
    for (const entry of entries) {
        if (!entry.isDirectory() || !entry.name.startsWith("phase-"))
...
function collectImplementationNotes(taskDir) {
    const entries = readdirSync(taskDir, { withFileTypes: true });
    for (const entry of entries) {
        if (!entry.isDirectory() || !entry.name.startsWith("phase-"))
            continue;
        ...
        const files = readdirSync(phaseDir);
        for (const file of files) {
            if (file.toLowerCase().includes("implementation") ||
