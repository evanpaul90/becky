...
function detectFrameworks(root) {
    const frameworks = [];
    // BMad
    const bmadDirs = ["_bmad", "_bmad-output"].filter((d) => existsSync(join(root, d)));
    ...
    // Hermes
    const hermesIndicators = [".hermes", "hermes.toml", "hermes"].filter((d) => existsSync(join(root, d)));
...
// ---------------------------------------------------------------------------
function readPlanningArtifacts(root) {
    const result = {
        prds: [],
        architecture: [],
        uxSpecs: [],
    ...
    const searchDirs = ["_bmad-output", "_bmad", "."].filter((d) => existsSync(join(root, d)));
        ...
        try {
            ...
            for (const file of allMd) {
                ...
                if (lower.includes("prd"))
                    result.prds.push(join(dir, file));
...
function countTests(root) {
    try {
        ...
        const testDirFiles = globSync("{tests,test,__tests__}/**/*.{ts,tsx,js,jsx}", {
            cwd: root,
            nodir: true,
            dot: false,
            ignore: ["node_modules/**"],
...
// ---------------------------------------------------------------------------
function extractClaudeMdRules(root) {
...
// ---------------------------------------------------------------------------
function parseSprintStatus(root, files) {
    if (files.length === 0)
        return null;
    const info = {
        done: 0,
            ...
            const walkForStatus = (obj, path) => {
                ...
                if (typeof record.status === "string") {
                    ...
                    if (status === "done" || status === "complete" || status === "completed") {
                        ...
                        info.blocked++;
                        const name = record.name || record.title || path;
...
// ---------------------------------------------------------------------------
function parseCheckboxCompletion(root, files) {
    let done = 0;
    let total = 0;
    for (const file of files) {
        const fullPath = join(root, file);
...
function generateSuggestions(report) {
    const suggestions = [];
    if (report.frameworks.some((f) => f.name === "BMad") &&
        report.artifacts.prds.length > 0) {
        suggestions.push("Run `becky learn _bmad-output/` to import your planning artifacts into Becky's wiki");
    }
    if (report.claudeMdRules.length > 0) {
        suggestions.push("Your CLAUDE.md has rules that should be migrated to Becky's `core/rules/` for proper compilation and enforcement");
    }
    if (report.sprintInfo && report.sprintInfo.blocked > 0) {
        suggestions.push(`Unblock ${report.sprintInfo.blocked} blocked stories first — blockers compound`);
    }
    if (report.sprintInfo && report.sprintInfo.inProgress > 0) {
        suggestions.push(`Finish the ${report.sprintInfo.inProgress} in-progress stories before starting new work`);
    }
    if (report.migrationCount > 0 && report.testCount === 0) {
        suggestions.push("You have migrations but zero test files — consider adding tests before the next migration");
    }
    if (report.frameworks.length === 0) {
        suggestions.push("No existing agent frameworks detected — run `/becky-greenfield` to start fresh with Becky");
    }
    if (suggestions.length === 0) {
        suggestions.push("Run `/becky-brownfield` to begin archaeology on this existing codebase");
        suggestions.push("Or run `/becky-greenfield` to start a new feature within this project");
    }
    return suggestions;
}
// ---------------------------------------------------------------------------
...
function formatReport(report) {
    const lines = [];
    const rule = (label) => "─".repeat(Math.max(label.length, 20));
    // Header
    lines.push("");
    lines.push("  Hey. I've been reading through your project. Here's what I see.");
    lines.push("");
    // PROJECT
    lines.push("  PROJECT");
    ...
    const sourceEntries = Object.entries(report.sourceFiles).sort((a, b) => b[1] - a[1]);
    ...
    // PLANNING ARTIFACTS
    const totalArtifacts = report.artifacts.prds.length +
    ...
    if (totalArtifacts > 0) {
        ...
        lines.push(`  PRDs: ${report.artifacts.prds.length}  |  Architecture: ${report.artifacts.architecture.length}  |  UX Specs: ${report.artifacts.uxSpecs.length}  |  Stories/Epics: ${report.artifacts.epicsStories.length}`);
        if (report.artifacts.incidents.length > 0) {
            lines.push(`  Incidents/Postmortems: ${report.artifacts.incidents.length}`);
        }
        if (report.artifacts.sprintStatus.length > 0) {
            lines.push(`  Sprint status files: ${report.artifacts.sprintStatus.join(", ")}`);
        }
        lines.push("");
    }
    // WHAT LOOKS DONE / PENDING
    if (report.sprintInfo) {
        const total = report.sprintInfo.done +
        ...
        lines.push(`  ${report.sprintInfo.done} stories done out of ${total} total (${total > 0 ? Math.round((report.sprintInfo.done / total) * 100) : 0}%)`);
        lines.push("");
        ...
        if (report.sprintInfo.blocked > 0) {
            lines.push(`  ${report.sprintInfo.blocked} BLOCKED`);
    ...
    if (report.claudeMdRules.length > 0) {
        ...
        if (report.claudeMdRules.length > 15) {
            lines.push(`  ... and ${report.claudeMdRules.length - 15} more rules`);
        }
        lines.push("");
    }
    // WHERE I'D START
    ...
    lines.push("");
    lines.push("  Ready when you are. Run /becky-greenfield or /becky-brownfield to begin.");
    lines.push("");
    return lines.join("\n");
}
function printReport(report) {
    const rule = (label) => chalk.dim("─".repeat(Math.max(label.length, 20)));
    console.log();
    console.log("  Hey. I've been reading through your project. Here's what I see.");
    console.log();
    // PROJECT
    ...
    if (sourceEntries.length > 0) {
        const fileSummary = sourceEntries
            ...
            .join(", ");
        const totalFiles = sourceEntries.reduce((sum, [, c]) => sum + c, 0);
        console.log(`  Source files:  ${chalk.bold(String(totalFiles))} total (${fileSummary})`);
    }
    if (report.migrationCount > 0) {
        console.log(`  Migrations:   ${chalk.bold(String(report.migrationCount))}`);
    }
    console.log(`  Tests:        ${report.testCount === 0 ? chalk.yellow("0") : chalk.bold(String(report.testCount))}`);
    if (report.envFiles.length > 0) {
        console.log(`  Env files:    ${chalk.dim(report.envFiles.join(", "))}`);
    }
    console.log();
    // EXISTING AGENT FRAMEWORKS
    if (report.frameworks.length > 0) {
    ...
    if (totalArtifacts > 0) {
        ...
        console.log(`  PRDs: ${chalk.bold(String(report.artifacts.prds.length))}  |  Architecture: ${chalk.bold(String(report.artifacts.architecture.length))}  |  UX Specs: ${chalk.bold(String(report.artifacts.uxSpecs.length))}  |  Stories/Epics: ${chalk.bold(String(report.artifacts.epicsStories.length))}`);
        if (report.artifacts.incidents.length > 0) {
            console.log(`  Incidents/Postmortems: ${chalk.yellow(String(report.artifacts.incidents.length))}`);
        }
        if (report.artifacts.sprintStatus.length > 0) {
            console.log(`  Sprint status files: ${chalk.dim(report.artifacts.sprintStatus.join(", "))}`);
        }
        console.log();
    }
    // WHAT LOOKS DONE / PENDING
    if (report.sprintInfo) {
        const total = report.sprintInfo.done +
            ...
            report.sprintInfo.pending +
            report.sprintInfo.blocked;
        const pct = total > 0 ? Math.round((report.sprintInfo.done / total) * 100) : 0;
        console.log(chalk.bold("  WHAT LOOKS DONE"));
        console.log(`  ${rule("WHAT LOOKS DONE")}`);
        console.log(`  ${chalk.green(String(report.sprintInfo.done))} stories done out of ${chalk.bold(String(total))} total (${chalk.bold(String(pct))}%)`);
        console.log();
        console.log(chalk.bold("  WHAT LOOKS PENDING"));
        console.log(`  ${rule("WHAT LOOKS PENDING")}`);
        console.log(`  ${chalk.yellow(String(report.sprintInfo.inProgress))} in progress`);
        console.log(`  ${chalk.dim(String(report.sprintInfo.pending))} pending`);
        if (report.sprintInfo.blocked > 0) {
            console.log(`  ${chalk.red(String(report.sprintInfo.blocked))} BLOCKED`);
            for (const detail of report.sprintInfo.details) {
    ...
    if (report.claudeMdRules.length > 0) {
        ...
        if (report.claudeMdRules.length > 15) {
            console.log(chalk.dim(`  ... and ${report.claudeMdRules.length - 15} more rules`));
        }
        console.log();
    }
    // WHERE I'D START
    ...
    console.log();
    console.log(`  Ready when you are. Run ${chalk.green("/becky-greenfield")} or ${chalk.green("/becky-brownfield")} to begin.`);
    console.log();
}
// ---------------------------------------------------------------------------
// Phase 6: Save the analysis
// ---------------------------------------------------------------------------
function saveReport(root, report, plainText) {
...
export async function run() {
    ...
    console.log();
    console.log(chalk.bold.magenta("  Becky scan") +
