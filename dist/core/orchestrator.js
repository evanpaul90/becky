...
function loadAgent(agentId) {
    ...
    return {
        id: frontmatter.id ?? primaryAgent,
        name: frontmatter.name ?? primaryAgent,
...
function readDirFiles(dirPath, extensions, skipPrefixes = ["_"]) {
...
function readDirFilesRecursive(dirPath, extensions) {
    if (!existsSync(dirPath))
        return [];
    const results = [];
    try {
...
function phaseNum(phaseKey) {
    return phaseKey.split("-")[0];
}
function phaseName(phaseKey) {
    return phaseKey.split("-").slice(1).join("-");
}
function phaseFolderName(phaseKey) {
    return `phase-${phaseKey}`;
}
function isoNow() {
    return new Date().toISOString();
}
function writeTaskYaml(taskDir, taskYaml) {
    writeFileSync(join(taskDir, "_task.yaml"), stringifyYaml(taskYaml, { lineWidth: 120 }), "utf-8");
}
function appendStatusLog(taskDir, entry) {
...
function gatherProjectConfig() {
    ...
    if (!existsSync(configPath))
        return "(No becky.config.yaml found.)";
    return safeReadFile(configPath);
}
function gatherPreviousPhaseOutputs(taskDir, phaseKeys, upToIndex) {
    const parts = [];
...
// Prompt Construction
// ---------------------------------------------------------------------------
function buildPrompt(agent, taskYaml, phaseKey, phase, taskDir, phaseKeys, phaseIndex, retryFeedback) {
...
// ---------------------------------------------------------------------------
function executeClaude(prompt, phaseDir, claudeCmd) {
...
// Gate Checks
// ---------------------------------------------------------------------------
function checkGate(output, agentId, _phase) {
...
// ---------------------------------------------------------------------------
function extractProposedRule(output) {
...
// ---------------------------------------------------------------------------
function generateMorningBrief(taskDir, taskYaml, phaseKeys, totalDurationMs) {
...
// Main Orchestrator
// ---------------------------------------------------------------------------
export async function orchestrate(taskDir, options) {
    ...
    let taskYaml = parseYaml(readFileSync(taskYamlPath, "utf-8"));
    ...
    for (let i = startIdx; i < phaseKeys.length; i++) {
        ...
        if (phase.status === "passed" || phase.status === "blocked")
            continue;
        ...
        console.log(chalk.bold(`  Phase ${num}`) +
        ...
        while (retryCount < maxRetries && !phasePassed) {
            ...
            if (gateResult.passed) {
                ...
                if (proposedRule)
                    phase.outputs.push("_proposed-rule.md");
    ...
    taskYaml = parseYaml(readFileSync(taskYamlPath, "utf-8"));
